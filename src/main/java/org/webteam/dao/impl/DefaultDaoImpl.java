...
import org.webteam.dao.DefaultDao;
...
public class DefaultDaoImpl<T> extends HibernateDaoSupport implements DefaultDao<T> {
		...
		if(clause != null &&clause.size()>0){
			for(Map.Entry<String, String> fieldClause : clause.entrySet()){
				...
				if(isEmpty.equalsIgnoreCase(op)){
					...
					criteria.add(Restrictions.like(fieldName, fieldValue==null?"":fieldValue.toString(), MatchMode.ANYWHERE));
	...
	public long queryCount(T t, Map<String, String> clause) {
		...
		return ((Number)(criteria.uniqueResult()==null?0:criteria.uniqueResult())).longValue();
	}
